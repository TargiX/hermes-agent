...
| `kanban_unblock` | Move blocked work to `ready` when all parents are done, or `todo` while any parent remains open. With `force=true` plus an audit reason, an orchestrator may also requeue the same `done` task after explicitly re-authorizing its continuation. Orchestrator-only; hidden from dispatcher-spawned task workers. | profile with `kanban` toolset |
