...
| `kanban_block` | Stop work and route by why: `kind=dependency` waits in `todo` and auto-resumes; `review_required` parks a frozen implementation for independent review without failure escalation; `needs_input`/`capability`/`transient` surface to a human. Repeated same-kind failure re-blocks auto-escalate to `triage`. | `reason` |
...
| `triage_recovery_assignee` | `""` | Optional operator profile that receives one idempotent recovery card when a repeated true blocker is routed to triage. Fresh intake triage is not affected. |
| `triage_recovery_per_tick` | `3` | Cap on new block-loop recovery cards per dispatcher tick. |
