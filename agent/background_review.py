...
_SKILL_REVIEW_PROMPT = (
    ...
    "the session contains a durable, validated learning. A no-op is the "
    "correct result when evidence is transient, speculative, duplicated, "
    "or merely a plausible unfinished attempt. Never invent a small update "
    "to demonstrate activity.\n\n"
    "Preserve higher-authority execution rules. A generalized lesson must "
    "not weaken or contradict an explicit user instruction, task contract, "
    "profile contract, repository rule, or the exact producer/consumer "
    "mechanism that was validated. Never replace a named canonical command, "
    "renderer, source of truth, or safety boundary with a generic recipe. "
    "When the lesson is valid only under one contract, state that scope in "
    "the skill; when unsure, return a no-op.\n\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "**Skills**: how to do this class of task. Update only from durable, "
    "validated learning. A no-op is correct for transient, speculative, "
    "duplicated, or unfinished evidence; never invent a small update merely "
    "to demonstrate activity.\n\n"
    "Preserve higher-authority execution rules. A generalized lesson must "
    "not weaken or contradict an explicit user instruction, task contract, "
    "profile contract, repository rule, or the exact producer/consumer "
