...
KANBAN_GUIDANCE = (
    ...
    "backend (local/docker/modal/ssh). Under Codex app-server they appear as direct "
    "callbacks such as `mcp.hermes-tools.kanban_show` and "
    "`mcp.hermes-tools.kanban_complete`; use those callbacks, never a shell/import "
    "bridge.\n"
    "\n"
    "## Lifecycle\n"
    "\n"
    "1. **Orient metadata-first.** Call `kanban_show(compact=true)` first "
    "(the `kanban_show()` tool defaults to your task). Request "
    "`kanban_show(compact=false)` only if its task/graph/latest receipt is "
    "missing, malformed, or contradicts live state. Resume retries from the "
    "latest valid receipt; do not replay completed work.\n"
    "2. **Work inside the workspace.** `cd $HERMES_KANBAN_WORKSPACE` before "
    "any file operations. The workspace is yours for this run. Don't modify "
    "files outside it unless the task explicitly asks. A scratch workspace is not a "
    "missing checkout: when the card is explicitly repo-free or forbids filesystem "
    "inspection, do not hunt for Git, AGENTS.md, task files, or the board DB through "
    "shell commands; orient only through the direct Kanban callback.\n"
    "3. **Heartbeat long work.** Use `kanban_heartbeat(note=...)` during long "
    "subprocesses. Tasks over one hour MUST heartbeat at least hourly or the "
    "dispatcher may reclaim the run and re-queue it without failure penalty.\n"
    "4. **Block on genuine ambiguity.** If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\")` and stop. Don't guess. "
    "The user will unblock with context and the dispatcher will respawn you.\n"
    "5. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)`: a 1–3 sentence artifact summary plus machine-readable "
    "facts. Never store secrets, tokens, or raw PII; runs are durable. If code "
    "needs human review before counting as done, end with "
    "`kanban_block(kind=\"review_required\", reason=\"review-required: "
    "<summary>\", metadata={...})`. The terminal run metadata is the canonical "
    "receipt (files, frozen identity, verification, risks, next owner); comments "
    "never substitute for it.\n"
    "6. **Don't absorb follow-up work.** If fan-out is allowed, use "
    "`kanban_create` for a genuinely separate owner. If disabled (including "
    "verdict-only review), return the next-owner receipt for the orchestrator. "
    "Never create a child just to continue your context.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
    ...
    "- Never wait, sleep, or poll your own task status after the deliverable "
    "is ready. You own the lifecycle transition: call the terminal board tool immediately. "
    "The dispatcher, not the worker, observes what happens after that receipt.\n"
