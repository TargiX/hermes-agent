...
def _apply_kanban_journal_mode(conn: sqlite3.Connection) -> None:
    """Enable WAL when supported and keep rollback mode as a safe fallback."""
...
def connect(
    ...
    *,
    ...
    WAL mode is established during serialized first-connect maintenance,
    with the shared SQLite helper retaining DELETE mode only on filesystems
    where WAL is unsupported.
...
def create_task(
    ...
    *,
    ...
    if project_id:
        ...
        project_resolution_error: Optional[Exception] = None
        try:
        ...
        if project_obj is None:
            if project_resolution_error is not None:
                raise ValueError(
                    f"could not resolve project {project_id}: "
                    f"{project_resolution_error}"
                ) from project_resolution_error
...
def validate_declared_handoff(
    *,
    ...
    """Validate immutable review/implementation receipts for every caller.

    This lives in the database boundary so the MCP tool, CLI, dashboard, and
    direct library callers cannot disagree about terminal handoff authority.
    """
...
def complete_task(
    ...
    *,
    ...
    handoff_error = validate_declared_handoff(
    ...
    * ``review_required`` — implementation is complete and frozen, but an
    ...
    with write_txn(conn):
        cur_row = conn.execute(
            "SELECT status, block_kind, block_recurrences, body FROM tasks WHERE id = ?",
            (task_id,),
        ).fetchone()
        if cur_row is None:
            return False
...
def _dispatch_once_locked(
    ...
    *,
    ...
    # pile up and time out. Both max_spawn and max_in_progress are live
    # concurrency caps, so combine them as a total cap. Do not replace
    # max_spawn with the number of remaining slots: the dispatch loop already
    # adds running_count when deciding whether another worker may start.
