...
def complete_task(
    ...
    initial_status = _task_status(conn, task_id)
    ...
    if initial_task is not None:
        proposed_payload = apply_kanban_transition_middleware(
            "complete",
            proposed_payload,
            task=asdict(initial_task),
            board=get_current_board(),
        ).payload
        result = proposed_payload.get("result")
        summary = proposed_payload.get("summary")
        metadata = proposed_payload.get("metadata")
    ...
    with write_txn(conn):
        ...
        if task_snapshot is not None:
            committed_payload = apply_kanban_transition_middleware(
                "complete",
                {
                    "result": result,
                    "summary": summary,
            ...
            if committed_payload != {
                ...
                raise RuntimeError(
                    "Kanban transition middleware changed the completion proposal "
...
def request_changes(
    ...
    metadata: Optional[dict] = None, expected_run_id: Optional[int] = None,
    ...
    with write_txn(conn):
        ...
        run_id = _end_run(
            conn, task_id, outcome="changes_requested", status=new_status,
