...
def apply_kanban_transition_middleware(
    ...
    **context: Any,
    ...
    """Apply plugin policy immediately before a Kanban state transition.

    A callback may replace the proposed payload with ``{"payload": {...}}``
    or reject it with ``{"decision": "deny", "reason": "..."}``. Callers
    invoke this while holding the Kanban write transaction, so a rejection
    cannot leave a partially-applied state transition.
    """
    from hermes_cli.plugins import has_middleware, invoke_middleware

    if not has_middleware(KANBAN_TRANSITION_MIDDLEWARE):
